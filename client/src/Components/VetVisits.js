...
const VetVists = (props) => {
  ...
  useEffect(() => {
    if (props.petI) setPetId(props.petI);
  }, [props.petI, setPetId]);
  ...
  const handleAddUpdateVisit = async (e, form, cb) => {
    ...
    const vals = {
      ...
      Weight: form.addVisitForm.Weight.value,
    };
          ...
          <ul className="overflow-auto">
            {visits.length > 0 ? visits.map((visit) => (
              ...
              >
                ...
                &nbsp; {visit.VisitNotes}
                {visit.Weight && <div className="text-muted small">Weight: {visit.Weight} lbs</div>}
              </li>
            )) : <p className="empty-list-msg">No vet visits recorded.</p>}
          </ul>
        </div>
        <button
          name="addVisitBtn"
